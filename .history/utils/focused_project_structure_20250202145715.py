...
def process_file(path, item, json_structure):
    ...
    elif item.endswith(".json"):  # Extract metadata from JSON files
        json_structure[item] = extract_json_keys(path)
    else:  # Non-Python files (e.g., LICENSE, README.md) will not have metadata
        json_structure[item] = None
...
output_folder = os.path.join(os.path.dirname(__file__), "utils")
