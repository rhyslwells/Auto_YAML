...
# ✅ Extract Python metadata (with decorators, arguments, and docstrings)
...
def process_file(path, item, json_structure):
    """Processes a file and extracts metadata if applicable."""
    if item.endswith(".py"):
    ...
    for item in sorted(os.listdir(root_dir)):
        ...
        if relative_path in focus_list:
            ...
            if os.path.isdir(path):
                ...
                process_file(path, item, json_structure)
    return tree_str, json_structure

# ✅ Multi-threaded metadata extraction for large projects
def process_files_parallel(files):
    """Processes multiple files in parallel to speed up metadata extraction."""
    with ThreadPoolExecutor() as executor:
        results = executor.map(lambda file: process_file(file[0], file[1], file[2]), files)
    return results
