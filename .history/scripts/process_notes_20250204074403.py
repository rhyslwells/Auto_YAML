...
def merge_yaml_headers(existing_yaml, new_yaml):
    """Merge YAML data, keeping existing values when AI returns empty ones."""
    merged_yaml = existing_yaml.copy()

    for key, value in new_yaml.items():
        if value is None or value == "":
            continue  # Ignore empty AI-generated fields

        if isinstance(value, list):  # Merge lists while removing duplicates
